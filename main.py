...
def train_epoch(data_loader, model, loss_fn, optimizer):
    ...
    for batch, (X, y,lengths) in enumerate(data_loader):
        X = X.float()
...
def main():
    ...
    def collate_fn(batch):
        ...
        return padded_seqs, labels, lengths[sorted_indices]
    ...
    test_loader = DataLoader(dev_data, batch_size = 512, shuffle=False, collate_fn=collate_fn)
    ...
    if args.model == "BOW":
        ...
        DAN_train_accuracy, DAN_test_accuracy = experiment(DAN(embeddings, hidden_size=100), train_loader, test_loader)

        # Plot the training accuracy
        plt.figure(figsize=(8, 6))
        plt.plot(DAN_train_accuracy, label='DAN layers')
        plt.xlabel('Epochs')
        plt.ylabel('Training Accuracy')
        plt.title('Training Accuracy for DAN Layer Networks')
        plt.legend()
        plt.grid()
        ...
        # Plot the testing accuracy
        plt.figure(figsize=(8, 6))
        plt.plot(DAN_test_accuracy, label='DAN layers')
        plt.xlabel('Epochs')
        plt.ylabel('Dev Accuracy')
        plt.title('Dev Accuracy for DAN Layer Networks')
        plt.legend()
        plt.grid()
